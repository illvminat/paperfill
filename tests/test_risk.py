...
from paperfill.execution import Fill, Order, Portfolio
...
def _open(token, price, size):
    return Order(id="r", token_id=token, side="BUY", price=D(price), size=D(size))
...
def test_limits_each_produce_a_reason():
    ...
    ok = e.check(Intent(M, UP, "BUY", D("0.5"), D("10")), {})
    assert ok.allowed and ok.reason is None
    assert "order notional" in e.check(Intent(M, UP, "BUY", D("0.5"), D("30")), {}).reason
    e.portfolio.apply(buy(UP, "0.5", "25"))
    assert "shares" in e.check(Intent(M, UP, "BUY", D("0.2"), D("10")), {}).reason
    assert "market notional" in e.check(Intent(M, DOWN, "BUY", D("0.5"), D("20")), {}).reason
    e2 = engine(max_market_notional="1000", max_total_exposure="25")
    e2.portfolio.apply(buy(UP, "0.5", "25"))
    d = e2.check(
        Intent(M, DOWN, "BUY", D("0.5"), D("10")), {UP: D("0.5")}, [_open(UP, "0.5", "20")]
    )
    assert "total exposure" in d.reason  # 12.5 held + 10 open + 5 new = 27.5 > 25
    assert "no shorting" in e.check(Intent(M, DOWN, "SELL", D("0.5"), D("1")), {}).reason
    assert e.check(Intent(M, UP, "SELL", D("0.5"), D("5")), {}).allowed
...
def test_daily_and_total_breakers_halt_and_block():
    ...
    assert "halted" in e.check(Intent(M, UP, "BUY", D("0.5"), D("1")), {}).reason
...
def test_pause_resume_and_kill_file(tmp_path):
    e = engine()
    e.pause()
    assert "paused" in e.check(Intent(M, UP, "BUY", D("0.5"), D("1")), {}).reason
    e.resume()
    assert e.check(Intent(M, UP, "BUY", D("0.5"), D("1")), {}).allowed
...
def test_resting_orders_count_toward_position_and_market_limits():
    e = engine(max_position_shares="30", max_market_notional="20", max_total_exposure="1000")
    resting = [_open(UP, "0.5", "25")]
    assert "resting 25" in e.check(Intent(M, UP, "BUY", D("0.5"), D("10")), {}, resting).reason
    assert "resting 12.5" in e.check(Intent(M, DOWN, "BUY", D("0.5"), D("20")), {}, resting).reason
    assert e.check(Intent(M, DOWN, "BUY", D("0.5"), D("10")), {}, resting).allowed
